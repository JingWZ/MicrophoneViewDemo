...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        microphoneLineWidth=6;
        mainPhoneSize.width=viewWidth/4;
        mainPhoneSize.height=viewHeight/2;
        distanceBetweenPhoneAndHolder=mainPhoneSize.width/1.5;
        verticalLineLength=viewHeight/10;
        horizontalLineLength=mainPhoneSize.width;
        microphoneLineColor=[UIColor grayColor];
        holderExtendingLength=0;
        
        colorType=kColorTypeYellowToRed;
        glowingType=kGlowingGradient;
        colorReverse=NO;
        colorCount=50;
...
- (void)drawRect:(CGRect)rect{
    ...
    centerYOfBottom=2.0/3*viewHeight-mainPhoneSize.width;
    ...
    bottomHeight=viewHeight-verticalLineLength;
    ...
    for (int i=0; i<=currentValue; i++) {
        ...
        switch (colorType) {
            case 0: //yellow->red
                ...
                color=CGColorCreate(colorSpace, (CGFloat[]){greenColorValue, greenColorValue, 1, 1});
                break;
            case 1: //cyan->blue
                color=CGColorCreate(colorSpace, (CGFloat[]){0, greenColorValue, 1, 1});
                break;
            case 2: //green->white
                color=CGColorCreate(colorSpace, (CGFloat[]){greenColorValue, 230/255.0, greenColorValue, 1});
                break;
            default:
                color=CGColorCreate(colorSpace, (CGFloat[]){1, greenColorValue, 0, 1});
                break;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    if ([keyPath isEqualToString:@"highlighted"]) {
        if (isPressed>0) {
            isPressed=-isPressed;
            [self setCurrentValueRate:0];
            [self setNeedsDisplay];
        }else{
            isPressed=-isPressed;
            [self setCurrentValueRate:1];
            [self setNeedsDisplay];
        }
            }
    if ([keyPath isEqualToString:@"currentValueRate"]) {
        [self setNeedsDisplay];
    }
